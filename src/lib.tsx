...
export const character = (letter: string = 'A', font: string = 'Product Sans') => {
    const comp = lazy(() => import(/* webapackMode: 'eager', webpackPreload: true */ `!@svgr/webpack!charset/${font}/${letter}.svg`));
    console.log(comp);
    return comp;
};

type CommonGenericLetterProps = {
    fontSize?: number,
...
class FallbackLetter extends PureComponent<FallbackLetterProps> {
    componentWillUnmount(): void {
        const { staggerDuration = 0, index, delay = 0, id } = this.props;
        setTimeout(() =>
...
export const Letter = ({
    ...
    return (
        <Suspense fallback={
            <FallbackLetter fontSize={fontSize}
